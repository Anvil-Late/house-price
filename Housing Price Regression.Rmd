...
```{r, echo=F}
load(url("https://github.com/Anvil-Late/house-price/raw/master/data/corrplot2.RData"))
```
...
```{r, echo=F}
load(url("https://github.com/Anvil-Late/house-price/raw/master/data/modfit_xgbm3.RData"))
```
...
scoring <- data.table(
   model = factor(c(
      "1.Ridge", "2.Lasso", "3.Elastinet", "4.Ridge + Lasso + Elnet",
      "5.SVM", "6.XGBM", "7.Ensemble", "8.XGBM + SVM", "9.XGBM + Lasso + SVM"
   )),
   accuracy = c(1-0.13859, 1-0.13631, 1-0.13903, 1-0.13648, 1-0.12872, 
                1-0.12716, 1-0.12437, 1-0.12321, 1-0.12485)
)
...
ggplot(data = scoring, aes(x = model, y = accuracy, fill = model)) + 
   ...
   geom_text(aes(1, 0.75, label = "Score : 0.13859", angle = 90))+
   geom_text(aes(2, 0.75, label = "Score : 0.13631", angle = 90))+
   geom_text(aes(3, 0.75, label = "Score : 0.13903", angle = 90))+
   geom_text(aes(4, 0.75, label = "Score : 0.13648", angle = 90))+
   geom_text(aes(5, 0.75, label = "Score : 0.12872", angle = 90))+
   geom_text(aes(6, 0.75, label = "Score : 0.12716", angle = 90))+
   geom_text(aes(7, 0.75, label = "Score : 0.12437", angle = 90))+
   geom_text(aes(8, 0.765, label = "Score : 0.12321 (best)", angle = 90))+
   geom_text(aes(9, 0.75, label = "Score : 0.12485", angle = 90))
