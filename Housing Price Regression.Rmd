...
library(parallel)
library(doParallel)
cluster1 <- makeCluster(detectCores() - 3)
...
```{r, eval=F}
modfit_xgbm <- xgb.train(dmat_train, params = xgbparam, nrounds = 2441)
```
...
write.csv(solution_7, "house_price_solution_11.csv", row.names = F)
write.csv(solution_8, "house_price_solution_12.csv", row.names = F)
...
- Ridge : 0.14391 / 0.14268
- Lasso : 0.16004 / 0.13968
- Elastinet : 0.14430 / 0.14251
- Ridge + Lasso + Elastinet : 0.14575 / 0.14078
- SVM : 0.23927 / 0.12872
- XGBM : 0.13287 / 0.13624
- Ensemble : 0.15334 / 0.12989
- Ensemble without SVM : 0.13284
- XGBM + SVM : 0.12711
- XGBM + LASSO + SVM : 0.12984
