...
train <- select(train, -c("YearBuilt", "YrSold"))
```

## Neighborhood

Neighborhood is one of the most important variable. However, it is not only categorical, but also has a lot of levels. To make it easier for our model to take neighborhood into account, we want to split it into 3 categories, poor, medium and expensive. Let's see if there is a clear separation that can be used to classify them :
...
ggplot(data = neig_sp_mean, aes(x = reorder(Neighborhood, mn), y = mn, fill = mn)) + 
   ...
   geom_text(aes(4.5, spmean, label = "Dataset mean price", vjust = -1, hjust = -0.1))+
