...
# Feature engineering + Removing variables 
test <- select(test, -c("Id", "GarageYrBlt"))
...
num_test <- select(num_test, -SalePrice)
...
preproc_test$logSalePrice <- log(test$SalePrice)

# for XGB predictions we convert prepro_test into a DMatrix
dmat_pptest <- xgb.DMatrix(data = as.matrix(select(preproc_test, -logSalePrice)))
