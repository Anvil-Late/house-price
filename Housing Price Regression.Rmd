...
qplot(GrLivArea, SalePrice, data = train) + geom_smooth(method = "lm", se = F)+
   geom_text_repel(
      aes(label = ifelse(train$GrLivArea[!is.na(train$SalePrice)]>4500, 
                         rownames(train), '')
          ))
```
...
qplot(TotalBsmtSF, SalePrice, data = train) + geom_smooth(method = "lm", se = F)+
   geom_text_repel(
      aes(label = ifelse(train$TotalBsmtSF[!is.na(train$SalePrice)] > 3000, 
                         rownames(train), '')
          ))
```
...
From only these 3 most important numeric variables, we can see that there are 
...
Moreover, GarageCars, GrLivArea and TotalBsmtSF are heavily correlated to other variables. We would therefore tend to remove them from the dataset. However, they are also heavily related to the response variable ; we might risk losing accuracy if we remove them. Instead, we will combine all surface variable into a single "total area" column and use that instead of all the others

```{r}
train[, HouseArea := TotalBsmtSF + ]
