...
```{r, eval=F}
modfit_rf <- caret::train(SalePrice ~ ., method = "rf", data = proc_train)
modfit_boost <- caret::train(SalePrice ~ ., method = "gbm", data = proc_train)
...
```{r}
comparison <- data.table(results = subtest$SalePrice, predicted = rf_predict)
RMSE(comparison$results, comparison$predicted, na.rm = T)
mean(comparison$results)
qplot(results, predicted, data = comparison) + geom_abline(slope = 1, intercept = 0, colour = "red")
```
