...
```{r, eval=F}
set.seed(69420)
modfit_svm <- svm(logSalePrice ~ ., data = model_train, method = "svmLinear", 
                  trControl = trctrl, 
...
```{r, eval=F}
modfit_xgbm2 <- xgb.train(dmat_train, params = xgbparam, nrounds = 2771)
```

```{r, echo=F}
load("./data/modfit_xgbm2.RData")
```

XGBoost models also provides us with feature importance. Let's see if there was any changes :
...
# Feature engineering + Removing variables 
subtest <- select(subtest, -c("Id", "GarageYrBlt"))
...
Let's check the RMSE for each predictor :
...
```{r}
comparison_table[, ensemble := (SP_lasso + 1.2*SP_svm + 4*SP_xgb)/6.2]
```
