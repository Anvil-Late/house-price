...
```
The correlation is exactly 1, which means our assumption was correct. What we'll do here is that we will combine *GrLivArea* and *TotalBsmtSF* into a single *TotalSF* variable. It might still be interesting to differentiate basement area and above ground area so we'll keep *GrLivArea* and *TotalsBsmtArea* but we will delete the rest :

```{r}
train[, TotalSF := GrLivArea + TotalBsmtSF]
train <- select(train, -c("X1stFlrSF", "X2ndFlrSF", "LowQualFinSF",  
                "BsmtFinSF1", "BsmtFinSF2", "BsmtUnfSF"))
```

For the remaining area variables, we'll also add categorical variables separating the low, med-low, med-high and high areas :

```{r}
bsmt_classify <- function(x){
   if (x <= 500){
      return(0)
   }else if (x %in% 501:1000){
      return(1)
   }else if (x %in% 1001:1500){
      return(2)
   }else if (x %in% 1501:2000){
      return(3)
   }else{
      return(4)
   }
...
train[, BsmtAreaClass := bsmt_classify(TotalBsmtSF), by = seq_len(nrow(train))]
train[, BsmtAreaClass := factor(BsmtAreaClass)]

grliv_classify <- function(x){
   if (x <= 1000){
      return(0)
   }else if (x %in% 1001:2000){
      return(1)
   }else if (x %in% 2001:3000){
      return(2)
   }else{
      return(3)
   }
...
train[, GrLivAreaClass := grliv_classify(GrLivArea), by = seq_len(nrow(train))]
train[, GrLivAreaClass := factor(GrLivAreaClass)]

totalsf_classify <- function(x){
   if (x <= 1000){
      return(0)
   }else if (x %in% 1001:2000){
      return(1)
      ...
      return(2)
   }else if (x %in% 3001:4000){
      return(3)
   }else if (x %in% 4001:5000){
      return(4)
   }else if (x %in% 5001:6000){
      return(5)
   }else{
      return(6)
   }
...
train[, TotalSFClass := totalsf_classify(TotalSF), by = seq_len(nrow(train))]
train[, TotalSFClass := factor(TotalSFClass)]
...
```{r}
lotfront_classify <- function(x){
   if (x <= 50){
      return(0)
   }else if (x %in% 51:100){
      return(1)
   }else if (x %in% 101:150){
      return(2)
   }else{
      return(3)
   }
...
train[, LotFrontageClass := lotfront_classify(LotFrontage), by = seq_len(nrow(train))]
train[, LotFrontageClass := factor(LotFrontageClass)]

lotarea_classify <- function(x){
   if (x <= 5000){
      return(0)
   }else if (x %in% 5001:10000){
      return(1)
   }else if (x %in% 10001:15000){
      return(2)
   }else if (x %in% 15001:20000){
      return(3)
   }else if (x %in% 20001:25000){
      return(4)
   }else{
      return(5)
   }
...
train[, LotAreaClass := lotarea_classify(LotArea), by = seq_len(nrow(train))]
train[, LotAreaClass := factor(LotAreaClass)]
...
```{r}
vnr_classify <- function(x){
   if (x <= 250){
      return(0)
   }else if (x %in% 251:500){
      return(1)
   }else if (x %in% 501:750){
      return(2)
   }else{
      return(3)
   }
...
train[, MasVnrAreaClass := vnr_classify(MasVnrArea), by = seq_len(nrow(train))]
train[, MasVnrAreaClass := factor(MasVnrAreaClass)]
...
In addition, we will complement the *YearRemodAdd* by a simpler variable, *RemodAdd* that will be 1 if the house has been remodeled and 0 if not
...
We'll also classify these date variables 

```{r}
year_classify <- function(x){
   if (x <= 1910){
      return(0)
   }else if (x %in% 1911:1950){
      return(1)
   }else if (x %in% 1951:1980){
      return(2)
   }else if (x %in% 1981:2000){
      return(3)
   }else{
      return(4)
   }
...
train[, YearBuiltClass := year_classify(YearBuilt), by = seq_len(nrow(train))]
train[, YearBuiltClass := factor(YearBuiltClass)]

train[, YearRemodAddClass := year_classify(YearRemodAdd), by = seq_len(nrow(train))]
train[, YearRemodAddClass := factor(YearRemodAddClass)]
```
...
with(train, cor(YearRemodAdd, YearBuilt))
```
We'll keep *GarageCars*, *TotalSF*, and *YearBuilt* that have a higher correlation with *SalePrice* and remove *GarageArea*, *TotRmsAbvGrd*, and *YearRemodAdd*



```{r}
train <- select(train, -c("GarageArea", "TotRmsAbvGrd", "YearRemodAdd"))
```

## Outliers

We saw during the exploratory data analysis that our dataset has several outliers. What we'll do is that for every numeric variable we'll drop the rows that are outside of the 1.7*IQR bracket :
...
train_original <- train_original[-row_remove, ]
for (i in 1:nrow(train_original)){
   if (is.na(train_original$LotFrontage[i])){
      ngbr <- train_original$Neighborhood[i]
      lf_mean <- neig_lf_mean[.(ngbr)]$meanLF # calling the mean with the key
      train_original$LotFrontage[i] <- lf_mean
   }
}
...
train_original <- select(train_original, -c("X1stFlrSF", "X2ndFlrSF", "LowQualFinSF", 
                "BsmtFinSF1", "BsmtFinSF2", "BsmtUnfSF"))
train_original[, BsmtAreaClass := bsmt_classify(TotalBsmtSF), by = seq_len(nrow(train_original))]
train_original[, BsmtAreaClass := factor(BsmtAreaClass)]
train_original[, GrLivAreaClass := grliv_classify(GrLivArea), by = seq_len(nrow(train_original))]
train_original[, GrLivAreaClass := factor(GrLivAreaClass)]
train_original[, LotFrontageClass := lotfront_classify(LotFrontage), by = seq_len(nrow(train_original))]
train_original[, LotFrontageClass := factor(LotFrontageClass)]
train_original[, LotAreaClass := lotarea_classify(LotArea), by = seq_len(nrow(train_original))]
train_original[, LotAreaClass := factor(LotAreaClass)]
train_original[, TotalSFClass := totalsf_classify(TotalSF), by = seq_len(nrow(train_original))]
train_original[, TotalSFClass := factor(TotalSFClass)]
train_original[, YearBuiltClass := year_classify(YearBuilt), by = seq_len(nrow(train_original))]
train_original[, YearBuiltClass := factor(YearBuiltClass)]
train_original[, YearRemodAddClass := year_classify(YearRemodAdd), by = seq_len(nrow(train_original))]
train_original[, YearRemodAddClass := factor(YearRemodAddClass)]
train_original[, MasVnrAreaClass := vnr_classify(MasVnrArea), by = seq_len(nrow(train_original))]
train_original[, MasVnrAreaClass := factor(MasVnrAreaClass)]
...
train_original <- select(train_original, -c("GarageArea", "TotRmsAbvGrd", "YearRemodAdd"))
...
num_train <- select(num_train, -c("YearBuilt", "YrSold", "MoSold", "SalePrice"))
...
```{r}
model_train <- data.table(cbind(normnum_train, train$YearBuilt, train$YrSold, train$MoSold, ohecat_train))
model_train$logSalePrice <- log(train$SalePrice)
```

And we drop the detected outliers :

```{r}
model_train <- model_train[-id_outliers, ]
```
...
set.seed(013969420)
modfit_lasso2 <- caret::train(logSalePrice ~ ., data = model_train, 
                        method = "glmnet",
                        trControl = trctrl,
                        tuneGrid = glmnetGrid)
...
```{r}
modfit_lasso2$bestTune
min(modfit_lasso2$results$RMSE)
```
...
>>>>>>> parent of 4f36b73... added lowvar dummy removal
...
tuned = tune.svm(logSalePrice ~ ., data = model_train, 
                 ...
                 cost = c(seq(0.1, 1, by = 0.1), seq(1, 10, by = 1), seq(10,100, by=20)),
                 coef0 = c(0.1, 1, 10),
...
```{r}
modfit_svm <- svm(logSalePrice ~ ., data = model_train, 
                  type = "eps",
                  kernel = "radial",
                  cost = 0.9,
                  gamma = 0.01,
...
=======
set.seed(69420)
modfit_svm2 <- svm(logSalePrice ~ ., data = model_train, cost = 3)
...
The accuracy is about the same as the lasso model.
...
=======
modfit_xgbm3 <- caret::train(logSalePrice ~ ., data = model_train,
>>>>>>> parent of 4f36b73... added lowvar dummy removal
                            method = "xgbTree", 
                            trControl = trctrl,
                            tuneGrid = xgbGrid)
...
```{r, eval=F}
<<<<<<< HEAD
modfit_xgbm2 <- xgb.train(dmat_train, params = xgbparam, nrounds = 1792)
=======
...
test <- select(test, -c("X1stFlrSF", "X2ndFlrSF", "LowQualFinSF", 
                "BsmtFinSF1", "BsmtFinSF2", "BsmtUnfSF"))
test[, BsmtAreaClass := bsmt_classify(TotalBsmtSF), by = seq_len(nrow(test))]
test[, BsmtAreaClass := factor(BsmtAreaClass)]
test[, GrLivAreaClass := grliv_classify(GrLivArea), by = seq_len(nrow(test))]
test[, GrLivAreaClass := factor(GrLivAreaClass)]
test[, LotFrontageClass := lotfront_classify(LotFrontage), by = seq_len(nrow(test))]
test[, LotFrontageClass := factor(LotFrontageClass)]
test[, LotAreaClass := lotarea_classify(LotArea), by = seq_len(nrow(test))]
test[, LotAreaClass := factor(LotAreaClass)]

test[, TotalSFClass := totalsf_classify(TotalSF), by = seq_len(nrow(test))]
test[, TotalSFClass := factor(TotalSFClass)]
test[, YearBuiltClass := year_classify(YearBuilt), by = seq_len(nrow(test))]
test[, YearBuiltClass := factor(YearBuiltClass)]
test[, YearRemodAddClass := year_classify(YearRemodAdd), by = seq_len(nrow(test))]
test[, YearRemodAddClass := factor(YearRemodAddClass)]
test[, MasVnrAreaClass := vnr_classify(MasVnrArea), by = seq_len(nrow(test))]
test[, MasVnrAreaClass := factor(MasVnrAreaClass)]
...
test <- select(test, -c("GarageArea", "TotRmsAbvGrd", "YearRemodAdd"))
...
num_test <- select(num_test, -c("YearBuilt", "YrSold", "MoSold"))
...
preproc_test <- data.table(cbind(normnum_test, test$YearBuilt, test$YrSold, test$MoSold, ohecat_test))
...
loganswer_xgbm <- predict(modfit_xgbm2, dmat_pptest)
